...
const GoodList = ({ navigation }) => {
    ...
    const { goodCart, items, system, itemType, shops } = useContext(Context)
    ...
    function GetGoodShop() {
        let temp = shops.filter(shop =>
            items.filter(item => system.goodType.includes(item.type))
                .map(i => i.shopId)
                .includes(shop.id))
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
                ...
                <View style={{
                    ...
                    <TouchableOpacity onPress={() => navigation.navigate('Home', { screen: 'Giỏ Hàng' })}>
                        ...
                        <View style={{
                            ...
                            <Text style={{ padding: 2, color: 'white' }}>{goodCart.reduce((acc, obj) => acc + obj.count, 0)}</Text>
                        </View>
                    </TouchableOpacity>
            ...
            {!isShowFilterShop ?
                ...
                <View>
                    <FlatList
                        horizontal={true}
                        data={itemType.filter(type => system.goodType.includes(type.name))}
            ...
            {isShowFilterShop ? null :
                <View style={{ flexDirection: 'row' }}>
                    <FlatList
                        numColumns={2}
                        data={items.filter(item => system.goodType.includes(item.type) && Search(item))}
